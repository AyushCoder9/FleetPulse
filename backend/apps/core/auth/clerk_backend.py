...
When CLERK_JWKS_URL and CLERK_ISSUER are set, validates Clerk RS256 session JWTs
and auto-creates Django users + organizations on first login.

Returns None when CLERK_JWKS_URL is absent (e.g. test env) so DRF returns 401.
"""
import logging
import time
...
class ClerkJWTAuthentication(BaseAuthentication):
    """
    DRF authentication class that validates Clerk RS256 session tokens.

    Reads: Authorization: Bearer <clerk-session-token>

    Returns None when Clerk is not configured (CLERK_JWKS_URL absent) so DRF
    falls through to the next handler (which returns 401 with no other classes).
    """
